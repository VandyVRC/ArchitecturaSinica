...
<!--    <xsl:import href="relationships.xsl"/>-->
 ...
 <!-- =================================================================== -->
    ...
    <xsl:param name="staticSitePath" select="'/Users/wsalesky/Vandy-tcadrt/staticSite/'"/>
    <xsl:param name="dataPath" select="'/Users/wsalesky/Vandy-tcadrt/tcadrt'"/>
        ...
        <xsl:for-each-group select="$path/child::*" group-by=".">
            <xsl:result-document href="{replace(.,'.xml','.html')}">
                <xsl:choose>
                    ...
                    <xsl:when test="$fileType = 'TEI'">
                        <xsl:message>Path: <xsl:value-of select="."/></xsl:message>
                        <xsl:call-template name="htmlPage">
            ...
            <body id="body">
                <xsl:choose>
                    <xsl:when test="not(empty($template))">
                        <xsl:choose>
                            <!-- div class="navbar -->
                            <xsl:when test="$template/descendant::html:div[@id='navbar']">
                                <xsl:copy-of select="$template/descendant::html:div[@id='navbar']"/>
                            </xsl:when>
                            <xsl:when test="$template/descendant::html:nav">
                                <xsl:copy-of select="$template/descendant::html:nav"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:message>No template found for html:head element 1</xsl:message>
                                <xsl:call-template name="genericNav"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:message>No template found for html:head element 2</xsl:message>
                        <xsl:call-template name="genericNav"/>
                    </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
