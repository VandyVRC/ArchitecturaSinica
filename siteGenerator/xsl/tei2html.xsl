<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:t="http://www.tei-c.org/ns/1.0" xmlns:x="http://www.w3.org/1999/xhtml" xmlns:saxon="http://saxon.sf.net/" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="http://syriaca.org/ns" exclude-result-prefixes="xs t x saxon local" version="2.0">
 ...
 <!-- =================================================================== -->
    ...
    <xsl:template match="t:relation">
        <!-- Hide relation element for Architectura Sinica
        <xsl:choose>
    ...
    <xsl:template match="t:TEI">
        ...
        <xsl:call-template name="otherDataFormats">
            ...
            <xsl:with-param name="formats" select="'print,tei,rdf'"/>
                ...
                <div class="row">
                    <div class="col-md-7">
                        <xsl:if test="/descendant::t:relation[@ref='foaf:depicts'][@ana='featured'] or /descendant::t:relation[@ref='foaf:depicts'][@rendition='featured']">
                            <xsl:for-each select="/descendant::t:relation[@ref='foaf:depicts'][@ana='featured'] | /descendant::t:relation[@ref='foaf:depicts'][@rendition='featured']">
                                <xsl:call-template name="getFlicker">
                                    <xsl:with-param name="imageURL">
                                        <xsl:choose>
                                            <xsl:when test="starts-with(@active,'http')"><xsl:value-of select="@active"/></xsl:when>
                                            <xsl:otherwise><xsl:value-of select="concat('https://',@active,'b.jpg')"/></xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:with-param>
                                    <xsl:with-param name="imageType" select="'featured-images'"/>
                                </xsl:call-template>
                            </xsl:for-each>
                        </xsl:if>
                        ...
                        <xsl:if test="/descendant::t:relation[@ref='foaf:depicts'][@ana!='featured']">
                            <xsl:for-each select="/descendant::t:relation[@ref='foaf:depicts'][@ana!='featured']">
                                <xsl:choose>
                                    <xsl:when test="@ana='featured'"/>
                                    <xsl:otherwise>
                                        <xsl:call-template name="getFlicker">
                                            <xsl:with-param name="imageURL">
                                                <xsl:choose>
                                            ...
                                            <xsl:with-param name="imageType" select="'thumb-images'"/>
                                        </xsl:call-template>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:for-each>
                    ...
                    <div class="col-md-5">    
                        <xsl:if test="/descendant::t:location[@type='gps']">
                       ...
                       <!-- Relationships
                        <div data-template="app:external-relationships" data-template-relationship-type="schema:containedInPlace" data-template-display="list-description" data-template-label="Contains artifact(s)"/>
                        <div data-template="app:internal-relationships" data-template-relationship-type="schema:containedInPlace" data-template-display="list-description" data-template-label="Contained in Place"/>
                        <div data-template="app:internal-relationships" data-template-relationship-type="dcterms:subject" data-template-display="list-description" data-template-label="Architectural Features"/>
                        --> 
                    </div>
                </div>
    </xsl:template>
    
    <!--WS:NOTE not done, need secure way to pickup flicker api key - Featured images  -->
    <xsl:template name="getFlicker">
        <xsl:param name="imageURL"/>
        <xsl:param name="imageType"/> 
        <!-- Need to store somewhere and build on the fly? Not sure -->
        <xsl:variable name="flickr-api-key" select="'HOLDING'"/>
        <xsl:variable name="imageID" select="tokenize($imageURL,'/')[last()]"/>
        <xsl:variable name="id" select="tokenize($imageID,'_')[1]"/>
        <xsl:variable name="secret" select="tokenize($imageID,'_')[2]"/>
        <xsl:variable name="request-url" select="concat('https://api.flickr.com/services/rest/?method=flickr.photos.getInfo&amp;api_key=',$flickr-api-key,'&amp;photo_id=',$id,'&amp;secret=',$secret)"/>
        <span class="{$imageType}">
    ...
    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
    <!-- Hide relationships in main content area -->
